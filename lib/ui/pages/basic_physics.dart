...
class BasicPhysics extends StatefulWidget {
  const BasicPhysics({super.key});

  @override
  State<BasicPhysics> createState() => _BasicPhysicsPageState();
}
...
class _BasicPhysicsPageState extends State<BasicPhysics> {
  ...
  Future<void> setup() async {
    ...
    threeJs.camera =
        three.PerspectiveCamera(60, threeJs.width / threeJs.height, 1, 10000);
    ...
    mats['ssph'] =
        three.MeshPhongMaterial.fromMap({'shininess': 10, 'name': 'ssph'});
    mats['sbox'] =
        three.MeshPhongMaterial.fromMap({'shininess': 10, 'name': 'sbox'});
    mats['scyl'] =
        three.MeshPhongMaterial.fromMap({'shininess': 10, 'name': 'scyl'});
    mats['ground'] = three.MeshPhongMaterial.fromMap({
      'shininess': 10,
      'color': 0x3D4143,
      'transparent': true,
      'opacity': 0.5
  ...
  void addStaticBox(
    ...
    mesh.rotation
        .set(rotation[0] * toRad, rotation[1] * toRad, rotation[2] * toRad);
  ...
  void populate(n) {
    ...
    for (int i = 0; i < max; i++) {
      ...
      three.Color randColor = three.Color.fromHex32(
      ...
      if (t == 1) {
        ...
        mat.color = randColor;
        cannon.Body sbody = cannon.Body(
            shape: cannon.Box(vmath.Vector3(w / 2, h / 2, d / 2).toCanonVec3()),
            position: vmath.Vector3(x, y, z).toCanonVec3(),
            mass: 1);
        bodys.add(sbody);
        world.addBody(sbody);
        ...
        cannon.Body sbody = cannon.Body(
            shape: cannon.Cylinder(
                radiusTop: w * 0.5, radiusBottom: w * 0.5, height: h),
            position: vmath.Vector3(x, y, z).toCanonVec3(),
            mass: 1);
        bodys.add(sbody);
        world.addBody(sbody);
    ...
    for (int i = 0; i < bodys.length; i++) {
      ...
      if (body.sleepState != cannon.BodySleepStates.sleeping) {
        ...
        if (mesh.position.y < -100) {
          ...
          z = -100 + math.Random().nextDouble() * 200;
          y = 100 + math.Random().nextDouble() * 1000;
          body.position = vmath.Vector3(x, y, z).toCanonVec3();
        }
      } else {
